...
`!timer list`
    ...
    delay_sec = 1,
    options = {},
    subcommands = { "add", "create", "new", "delete", "remove", "interval", "edit", "rename", "list", "view", "call" },
    aliases = {},
    ...
    handle = function(request)
        ...
        if scid == "list" then
            local cmds = db_query('SELECT name FROM timers WHERE room_id = $1', { request.room.id })
            local names = {}
            for i = 1, #cmds, 1 do
        ...
        local timers = db_query(
            'SELECT id, name, interval, message FROM timers WHERE name = $1 AND room_id = $2',
        ...
        if scid == "new" or scid == "add" or scid == "create" then
            ...
            db_execute('INSERT INTO timers(room_id, name, interval, message) VALUES ($1, $2, $3, $4)',
                { request.room.id, name, interval_sec, message })
        ...
        elseif scid == "rename" then
            ...
            local new_timers = db_query('SELECT id FROM timers WHERE name = $1 AND room_id = $2',
                { new_name, request.room.id })
        ...
        elseif scid == "interval" then
            ...
            db_execute('UPDATE timers SET interval = $1 WHERE id = $2',
        ...
        elseif scid == "view" then
            return l10n_custom_formatted_line_request(request, lines, "view",
                { timer.id, timer.name, time_humanize(tonumber(timer.interval)), timer.message })
