local lines = {
    english = {
        ...
        ["user_changes_more"] =
        "{sender.alias_name}: %s was previously known as %s (%s ago) and has %s other changes. More: %s",
    },
    russian = {
        ...
        ["user_changes_more"] =
        "{sender.alias_name}: %s был известен как %s (%s назад), а также имеет ещё %s изменений. Больше: %s",
    },
}
...
+ Name history is taken from the third-party API service ["logs.zonian.dev"](https://logs.zonian.dev)
    ...
    handle = function(request)
        ...
        local previoustimestamp = time_humanize(time_current() -
            time_parse(remove_ms(previousname.last_timestamp), "%Y-%m-%dT%H:%M:%S"))

        if #usernames == 2 then
            return l10n_custom_formatted_line_request(request, lines, "user_changes_one",
                { name, previousname.user_login, previoustimestamp })
        end
        ...
        for i = #usernames, 1, -1 do
            ...
            body = body ..
                tostring(#usernames - i + 1) ..
                ". " .. user.user_login .. " (last seen " .. hts .. " ago, on " .. fts .. ")\r\n"
        end

        local time = time_format(time_current(), "%d.%m.%Y %H:%M:%S %z")

        local link = paste_upload(body, name .. "'s name history as of " .. time)

        return l10n_custom_formatted_line_request(request, lines, "user_changes_more",
            { name, previousname.user_login, previoustimestamp, tostring(#usernames - 2), link })
    end,
}
